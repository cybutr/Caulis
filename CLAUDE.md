...
- **Secret "Layout Lab"** (`GardenHeroBanner`): tapping the hero banner 7× (same discovery shape as the Developer panel's 7-tap version row, no PIN — this only repositions a decorative element) sets `caulis_layout_lab_unlocked` and shows a one-time toast. `onPointerUp` (tap counting) fires before the browser's own `click` (which opens Plant Detail) on every tap, so the first tap in a burst still opens normally, but the 2nd+ rapid tap within the same 1.4s counting window is suppressed from opening (`suppressOpen` ref) — otherwise every tap reopened Plant Detail before a second tap could ever land, making 7 taps unreachable. Once unlocked, long-pressing (480ms) the hero enters a real drag mode (dashed sage glow + "Layout Lab" pill + Done chip): dragging it up past the header moves it to `above`, dragging down returns it to `below`, persisted through the same `onSetHeroBanner`/`caulis_hero_banner` as the plain Settings toggle. This is additive — the Settings toggle stays the simple, discoverable path; Layout Lab is an extra secret layer for whoever finds it.
