...
async function pushSubscribe(subscription, wateringEnabled, digestEnabled, lang, reminderHourUtc, digestDayOfWeek, customRemindersEnabled, wateringFrequencyDays, pushTimesPerDay) {
  const token = getActiveToken();
  if (!token) return false;
  const { ok } = await _api('/api/push/subscribe', {
    method: 'POST',
    body: JSON.stringify({ subscription, wateringEnabled, digestEnabled, lang, reminderHourUtc, digestDayOfWeek, customRemindersEnabled, wateringFrequencyDays, pushTimesPerDay }),
  }, token);
  return ok;
}

async function pushSetPrefs(endpoint, wateringEnabled, digestEnabled, lang, reminderHourUtc, digestDayOfWeek, customRemindersEnabled, wateringFrequencyDays, pushTimesPerDay) {
  const token = getActiveToken();
  if (!token) return false;
  const { ok } = await _api('/api/push/prefs', {
    method: 'PUT',
    body: JSON.stringify({ endpoint, wateringEnabled, digestEnabled, lang, reminderHourUtc, digestDayOfWeek, customRemindersEnabled, wateringFrequencyDays, pushTimesPerDay }),
  }, token);
  return ok;
}
