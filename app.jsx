...
function App() {
  ...
  const setDarkMode = (v) => {
    ...
    setDarkModeState(v);
    trackSeenValue('caulis_seen_modes', v ? 'dark' : 'light');
  };
  ...
  }, [darkMode, palette, customPaletteColor]);
  useEffect(() => {
...
window.onload=()=>{
  ...
  if (tab === 'needs')    screen = <NeedsWaterScreen plants={plants} onOpen={id=>openDetail(id)} onLongPress={p=>setMenuPlant(p)} onSnooze={snooze} onWaterAll={waterAll} onWaterOne={waterOne} confirmDelete={confirmDelete} czechMode={identifyLang === 'cs'} gardenName={gardenName} reduceMotion={reduceMotion} {...screenProps}/>;
