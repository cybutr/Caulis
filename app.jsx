...
function App() {
  ...
  const setPushToggle = async (kind, value) => {
    ...
    try {
      ...
      if (nextWatering || nextDigest) {
        ...
        const ok = await pushSubscribe(sub.toJSON(), nextWatering, nextDigest, identifyLang === 'cs' ? 'cs' : 'en', localHourToUtc(reminderHourLocal), digestDay, customRemindersEnabled, wateringFrequencyDays);
  ...
  const syncPushPrefs = async () => {
    ...
    const reg = await navigator.serviceWorker.ready;
    const sub = await reg.pushManager.getSubscription();
    if (sub) pushSetPrefs(sub.endpoint, pushWatering, pushDigest, identifyLang === 'cs' ? 'cs' : 'en', localHourToUtc(reminderHourLocal), digestDay, customRemindersEnabled, wateringFrequencyDays);
  };
  ...
  useEffect(() => { syncPushPrefs(); }, [identifyLang, reminderHourLocal, digestDay, customRemindersEnabled, wateringFrequencyDays]);
