    ...
    try {
      ...
      if (en) {
        ...
        const match = PERENUAL.find(p => {
          ...
          return sn.some(s => s.toLowerCase() === scientificName.toLowerCase());
        }) || PERENUAL.find(p => p.common_name?.toLowerCase() === englishName.toLowerCase());
        if (match) return { ...match, _source: 'PlantNet' };  // library hit carries czech_names already
